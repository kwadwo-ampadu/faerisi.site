...
const FormUpload = forwardRef<HTMLInputElement, Props>(({ name, label, accept, required, labelProps }, ref) => {
    ...
    const thumbs = files?.map((file: { name: string; preview: string | undefined }) => (
        <Box w={'full'} key={file.name} pos="relative">
            ...
            {file?.preview ? (
                ...
                <HStack spacing="1rem" align="center" justify="space-between" w="full" overflow="hidden">
    ...
    return (
        <FormControl ref={ref} isRequired={required} onMouseLeave={() => isTouched}>
